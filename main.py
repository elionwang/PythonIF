...
while i <= matches: # maak while loop, voer uit als i kleiner of gelijk is aan variabele 'matches'
  ...
  if spelerInput < 0 or spelerInput > 2: # voer code uit als variabele spelerInput kleiner dan 0 of groter dan 2 is
    i -= 1 # verminder i met 1 zodat het gewenste aantal potjes wordt gespeeld, i += 1 in laatste line van while loop
    print(f'\x1b[4;30;41m{spelerInput} is geen geldige input idioot. Kies steen[0], papier[1] of schaar[2].',"\033[0m") # print error message en beledig user, gebruik ANSI escape code voor rode achtergrond van tekst en op het einde zodat tekst daarna wit blijft
